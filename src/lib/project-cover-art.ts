/** Génère une cover SVG unique par slug (gradient abstrait, sans titre — évite le doublon avec la card). */
...
/**
 * Cover décorative uniquement : pas de titre / sous-titre lisibles.
 * Le titre reste affiché une seule fois dans la card HTML.
 */
export function buildProjectCoverSvg(title: string, slug: string, _subtitle?: string): string {
  const { c1, c2, c3 } = slugPalette(slug)
  const mono = escapeXml(initials(title))
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="750" viewBox="0 0 1200 750">
  ...
  <text x="80" y="560" font-family="system-ui, sans-serif" font-size="160" font-weight="800" fill="rgba(255,255,255,0.07)">${mono}</text>
  <rect x="80" y="610" width="120" height="4" rx="2" fill="${c3}"/>
</svg>`
}
